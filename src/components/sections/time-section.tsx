...
const generateInitialDocumentStates = (count = 15) => {
  return Array.from({ length: count }).map((_, index) => ({
    id: `doc-${index}`, // Stable ID for React key
...
export default function TimeSection() {
  ...
  useEffect(() => {
    ...
    if (isOn) {
      // When turning ON:
      // The goal is to transition from floating (sucked: false) to sucked-in (sucked: true).
      // The setTimeout helps ensure the browser registers the class/style changes
      // for the transition to occur from the current visual state.
      timerId = setTimeout(() => {
        setDocumentPositions(prevPositions =>
          prevPositions.map(p => ({ ...p, sucked: true }))
        );
      }, 50); // A small delay (e.g., 20-50ms)
    } else {
      // When turning OFF:
      // If any files were sucked, reset them to new floating positions.
      const wasOn = documentPositions.some(p => p.sucked);
      if (wasOn || documentPositions.length === 0 || documentPositions.every(p => p.sucked)) { // Added more robust check
        setDocumentPositions(generateInitialDocumentStates());
      } else {
        // If already off and files are floating, ensure 'sucked' is false for all
        // This case might occur if the component re-renders while already off.
        setDocumentPositions(prevPositions =>
          prevPositions.map(p => p.sucked ? { ...p, sucked: false } : p)
        );
      }
    }
  ...
  }, [isOn, clientRendered]); // documentPositions itself is not a dependency here to prevent loops
      ...
      <div className="container mx-auto px-4 text-center">
        ...
        <div className="absolute inset-0 w-full h-full" aria-hidden="true">
          {clientRendered && documentPositions.map((pos, index) => (
            <FileText
              key={pos.id} // Use stable ID
              className={cn(
                'absolute text-foreground/80',
                // Apply transition properties explicitly
                'transition-[opacity,transform,top,left]',
                pos.sucked
                  ? 'duration-[1200ms] ease-in' // Duration and easing for suck-in
                  : 'animate-subtle-float' // Floating animation
              )}
              size={40 * pos.scale} // Base size, effective scale comes from pos.scale
              style={
                pos.sucked ? {
                  top: '50%',
                  left: '50%',
